...
QString GraphicsWidget::SearchListIndex(int RIDX)
{
    //qDebug() << "SearchListIndex << Search item: " << RIDX;
    int count = ui->tagTable->rowCount();
    for(int i = 0; i < count; i++)
    {
       QTableWidgetItem* item = ui->tagTable->item(i,ColumnIDr);
       //qDebug() << "SearchListIndex << IDrString: " << item->text();
       QString IDstring = item->text();
       QString ID = QString::number(RIDX, 16);
       //qDebug() << "SearchListIndex << RIDX converted into: " << ID;
       //qDebug() << "SearchListIndex << IDr: " << IDstring[2];
       if(ID[0] == IDstring[2])
       {
        //qDebug() << "SearchListIndex << Label Found";
        item = ui->tagTable->item(i,ColumnID);
        //qDebug() << "SearchListIndex << text: " << item->text();
        return item->text();
       }
    }
    qDebug() << "SearchListIndex << Label not found";
    return NULL;
}
...
void GraphicsWidget::tagTableChanged(int r, int c)
{
    if(!_ignore)
    {
        ...
        if(c == ColumnID) //label has changed
        {
            ...
            //update the map
            qDebug() << "Searching tag: " << tagId << " in labelarray....";
            QMap<quint64, QString>::iterator i = _tagLabels.find(tagId);
            RTLSDisplayApplication::mainWindow()->viewSettingsWidget()->TagLabelChanged(r,newLabel);
            if(i == _tagLabels.end()) //did not find the label
            {
                qDebug() << "label not found: " << tagId;
        ...
        switch(c)
        {
            ...
            case ColumnZ -1:
            {
                ...
                if(ok)
                {
                    ...
                    double xn = (ui->anchorTable->item(r,ColumnX -1)->text()).toDouble(&ok);
                    double yn = (ui->anchorTable->item(r,ColumnY -1)->text()).toDouble(&ok);
                    double zn = (ui->anchorTable->item(r,ColumnZ -1)->text()).toDouble(&ok);

                    ui->anchorTable->item(r,ColumnX -1)->setText(QString::number(xn, 'f', 2));
                    ui->anchorTable->item(r,ColumnY -1)->setText(QString::number(yn, 'f', 2));

                    anchPos(r, xn, yn, zn, true);
                    ...
                    if((ColumnZ -1 == c) && (r <= 2))
                    {
                        ...
                        ui->anchorTable->item(0,ColumnZ -1)->setText(QString::number(zn, 'f', 2));
                        ui->anchorTable->item(1,ColumnZ -1)->setText(QString::number(zn, 'f', 2));
                        ui->anchorTable->item(2,ColumnZ -1)->setText(QString::number(zn, 'f', 2));
                    }
                }
            }
            break;
...
void GraphicsWidget::insertTag(int ridx, QString &t, bool showR95, bool showLabel, QString &l)
{
    ...
    for( int col = ColumnID ; col < ColumnCount; col++)
    {
...
void GraphicsWidget::insertAnchor(int ridx, double x, double y, double z,int *array, bool show)
{
    ...
    ui->anchorTable->setItem(ridx, ColumnX -1, itemx);
    ui->anchorTable->setItem(ridx, ColumnY -1, itemy);
    ui->anchorTable->setItem(ridx, ColumnZ -1, itemz);
        ...
        if (tag)
        {
            ...
            //update Tag/Anchor range value in the table
            {
                ...
                else //add tag into tag table
                {
                    ...
                    int count =_tagLabels.count();
                    if(count >= 1)
                    {
                        if(_tagLabels.contains(tagId))
                        {
                            QMap<quint64, QString>::iterator i = _tagLabels.find(tagId);
                            //qDebug() <<"Label: " << i.value();
                            //qDebug() <<"ridx: " << findTagRowIndex(t);
                            ui->tagTable->item(findTagRowIndex(t),ColumnID)->setText(i.value());
                            RTLSDisplayApplication::mainWindow()->viewSettingsWidget()->TagLabelChanged(tagId,i.value());
                        }
                    }
                }
            }
