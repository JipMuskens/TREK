...
typedef struct{
	...
	uint32 status;			//initial value of status register
	uint8 event;
    uint8 aatset;
	uint16 datalength;
 ...
 *
 *  Description: This is used to return the read device type and revision information of the DW1000 device (MP part is 0xDECA0130)
 *
 * input parameters:
 *
 * output parameters:
 ...
 *
 */
uint16 dwt_getTREKOTPantennadelay(uint8 anchor, uint8 chan, uint8 datarate);
 ...
 * input parameters
 * @param gpioNum    -   this is the GPIO to configure - see GxM0... GxM8 in the deca_regs.h file
 * @param direction  -   this sets the GPIO value - see GDP0... GDP8 in the deca_regs.h file   
 *
 * output parameters
 *
 * no return value
